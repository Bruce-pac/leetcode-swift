...
class Solution {
    func wordBreak(_ s: String, _ wordDict: [String]) -> Bool {
        let r1 = rewordBreak(s, wordDict)
        let r2 = rewordBreak(s, wordDict.reversed())
        return r1 || r2
    }
    func rewordBreak(_ s: String, _ wordDict: [String]) -> Bool {
        for word in wordDict {
            if s.contains(word) {
                let subrange = s.range(of: word)
            let substr = s.replacingCharacters(in: subrange!, with: " ")
                let str = substr.replacingOccurrences(of: " ", with: "")
                if str.isEmpty {
                    return true
                }else {
                    let subWordDict = wordDict.drop { $0 == word }
                    return rewordBreak(substr, Array(subWordDict))
                }
            }else {
                continue
            }
